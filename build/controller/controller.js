...
export class Controller {
    ...
    recursiveTrace(x = Math.floor(this._prevCoordinate.x / BLOCK_WIDTH), y = Math.floor(this._prevCoordinate.y / BLOCK_HEIGHT), xOf = 1, yOf = 1) {
        if (xOf === 1) {
        ...
        if (curGridOnTrace)
            ...
            if (this._ball.y < this._prevCoordinate.y)
                ...
                yOf = 0;
            if (xOf === 0 || yOf === 0)
                return this.recursiveTrace(x + xOf, y + yOf, xOf, yOf);
            if (this.ballTrack(x + xOf, y) <= y + yOf)
                return this.recursiveTrace(x + xOf, y, xOf, yOf);
            else
                return this.recursiveTrace(x, y + yOf, xOf, yOf);
        }
    }
    judgeBallCrash() {
