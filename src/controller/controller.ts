...
import {
  BlockType,
  Block,
  BlockAdditionalSetting,
  ColorFuncByTimeStamp,
  PaddingFuncByTimeStamp,
} from "../block/baseBl.js";
import {
  CANVAS_WIDTH,
  CANVAS_HEIGHT,
  BLOCK_WIDTH,
...
export class Controller {
  ...
  private _keyPressedTimeStamp: {
    [k in Extract<BallDirection, "left" | "right">]: DOMHighResTimeStamp;
  };
  ...
  private checkAndConfirm(
    grid: Block<BlockType> | null | undefined,
    array: CrashInfo[],
    dir: BallDirection
  ) {
    if (grid) {
  ...
  private ballCrashInfo(
    h_d: "left" | "right" | "center",
    v_d: "up" | "down" | "center"
  ): CrashInfo[] | false {
    ...
    switch (h_d) {
      case "right":
        if (v_d === "up") {
          for (
            let x = Math.floor((this._prevCoordinate.x + xOf) / BLOCK_WIDTH);
            x <= Math.floor((this._ball.x + xOf) / BLOCK_WIDTH);
            x++
          ) {
            const y = this.marginBallTrack(x * BLOCK_WIDTH, 0, "down");
            if (
              y <= this._prevCoordinate.y + this._ball.r &&
              ...
              this.checkAndConfirm(
                this._map.matrix[Math.floor(y / BLOCK_HEIGHT)] &&
                  this._map.matrix[Math.floor(y / BLOCK_HEIGHT)][x],
                crashArray,
                "right"
              );
            }
          ...
          for (
            ...
            if (
              ...
              this.checkAndConfirm(
                this._map.matrix[Math.floor(y / BLOCK_HEIGHT)] &&
                  this._map.matrix[Math.floor(y / BLOCK_HEIGHT)][x - 1],
                crashArray,
                "up"
              );
    ...
    //down
    this.checkAndConfirm(
      this._map.matrix[Math.floor(m_down / BLOCK_HEIGHT)] &&
        this._map.matrix[Math.floor(m_down / BLOCK_HEIGHT)][
    ...
    //up
    this.checkAndConfirm(
      this._map.matrix[Math.floor(m_up / BLOCK_HEIGHT)] &&
        this._map.matrix[Math.floor(m_up / BLOCK_HEIGHT)][
          Math.floor(this._ball.x / BLOCK_WIDTH)
        ],
      crashArray,
    ...
    //left
    this.checkAndConfirm(
      this._map.matrix[Math.floor(this._ball.y / BLOCK_HEIGHT)] &&
        this._map.matrix[Math.floor(this._ball.y / BLOCK_HEIGHT)][
          Math.floor(m_left / BLOCK_WIDTH)
        ],
      crashArray,
      "left"
    );
    ...
    this.checkAndConfirm(
      this._map.matrix[Math.floor(this._ball.y / BLOCK_HEIGHT)] &&
        this._map.matrix[Math.floor(this._ball.y / BLOCK_HEIGHT)][
          Math.floor(m_right / BLOCK_WIDTH)
        ],
      crashArray,
      "right"
    );
  ...
  judgeBallCrash(time: DOMHighResTimeStamp) {
    let h_d: "left" | "right" | "center";
    let v_d: "up" | "down" | "center";
      ...
      switch (c.dir) {
        case "left":
          if (this._keyObserver.right) {
            if (
              time - this._keyPressedTimeStamp.left <
        ...
        case "right":
          if (this._keyObserver.left) {
            if (
              time - this._keyPressedTimeStamp.right <
              Controller._jumpableTimeGap
            )
              jumpOnTheWall = true;
          }
  ...
  private bounceAudioPlay(type: BlockType | SpecialBallMove) {
    ...
    //     audio.play();
    // }
    //bug - delayed audio play when deployed (github page)
  }
  ...
  private updateBlockPropertyByCrash(info: CrashInfo) {
    switch (info.block.type) {
      case "Fragile":
        if (info.dir === "down")
          this._map.deleteBlock(
            info.block.x / BLOCK_WIDTH,
            info.block.y / BLOCK_HEIGHT
          );
        break;
      case "End":
        this._map.initializeMatrix();
        this.toNextStage();
        const clear = stageBnd[this._stage](
          this.generateBlock.bind(this),
          this.initializeBall.bind(this)
        );
        if (clear) {
          this._clear = true;
    ...
    this._map.matrix.forEach((row) => {
      row.forEach((e) => {
        if (e) {
          if (e.type === "End") {
            context.fillStyle = (
              e.renderSetting.outerColor as ColorFuncByTimeStamp
            )(time);
          } else {
  ...
  generateBlock<T extends BlockType>(
    x: number,
    y: number,
    type: BlockType,
    opt?: BlockAdditionalSetting<T>
  ...
  ball_h_move() {
    ...
    if (
      (this._keyObserver.left && this._ball.flyStatus === "FlyRight") ||
