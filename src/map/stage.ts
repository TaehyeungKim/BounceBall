...
export const stageBnd: StageGenerator = {
    0: (generator, initializer)=>{
        ...
        for(let k = 0; k <= h_end; k++) {
            if(k < 26 || k > 28) generator(k, v_end,  'Normal')
            if(k < 26 || k > 28) generator(k, v_end-1,  'Normal')
            if(k > 7) if(k < 26 || k > 28) generator(k, v_end-2,  'Normal')
            if (k > 11) if(k < 26 || k > 28) generator(k, v_end-3,  'Normal')
            if(k>15) if(k < 26 || k > 28) generator(k, v_end-4,  'Normal')
            if (k > 19) if(k < 26 || k > 28) generator(k, v_end-5,  'Normal')
            if(k>23) {
                if(k < 26 || k > 28) {
